...
<div class="sc_courses_item trx_addons_hover trx_addons_hover_style_links">
	<?php if (has_post_thumbnail()) { ?>
		<div class="sc_courses_item_thumb">
			<?php the_post_thumbnail( trx_addons_get_thumb_size($args['columns'] > 2 ? 'big' : 'huge'), array('alt' => get_the_title()) ); ?>
			...
			<div class="sc_courses_item_meta">
				...
				*/
				...
				<?php if (($excerpt = get_the_excerpt()) != '') { ?>
					<div class="sc_courses_item_meta_description"><?php echo esc_html($excerpt); ?></div>
				<?php } ?>
      </div>
		</div>
	...
	<div class="trx_addons_hover_content">
		...
		<div class="trx_addons_hover_links">
			<a href="<?php echo esc_url(get_permalink()); ?>" class="trx_addons_hover_link"><?php esc_html_e('More Info', 'translang'); ?></a>
			<?php if (!empty($meta['product']) && (int) $meta['product'] > 0) { ?>
			<a href="<?php echo esc_url(get_permalink($meta['product'])); ?>" class="trx_addons_hover_link2"><?php esc_html_e('Buy Now', 'translang'); ?></a>
			<?php } ?>
		</div>
	</div>
</div>
