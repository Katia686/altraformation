...
<div class="sc_services_item<?php
	...
	echo empty($args['featured']) || $args['featured']=='image' || $args['featured']=='image_icon'
					? ' with_image'
...
?>">
	...
	} else if ( (isset($args['featured']) && $args['featured']=='image_icon') && !empty($meta['image_icon'])) {
	?>

	 <?php
	} else if ( (isset($args['featured']) && $args['featured']=='icon') && !empty($meta['icon'])) {
		$svg = '';
	...
	<div class="sc_services_item_info">
		<div class="sc_services_item_header">
			<h5 class="sc_services_item_title">
				<?php
				if ( (isset($args['featured']) && $args['featured']=='image_icon') && !empty($meta['image_icon'])) { ?>
				<span class="sc_services_item_title_number">
					<?php
						$number = get_query_var('trx_addons_args_item_number');
						printf("%02d", $number);
					?>
				...
				<?php } ?>
				<a href="<?php echo esc_url($link); ?>" class="sc_services_item_image_icon"><img src="<?php echo esc_url($meta['image_icon']); ?>" alt="<?php esc_html__('image', 'translang'); ?>"/></a>
				<a href="<?php echo esc_url($link); ?>"><?php the_title(); ?></a></h5>
		...
		<?php if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) { ?>
			...
			<div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="sc_button sc_button_default sc_button_size_normal sc_button_hover_slide_left"><?php esc_html_e('Read more', 'translang'); ?></a></div>
		<?php } ?>
	</div>
</div>
<?php
