...
?>><?php
	...
	if (!empty($args['image'])) {
		...
		?><img src="<?php echo esc_url($args['image']); ?>" alt="<?php esc_html__('image', 'translang'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
		if (!empty($args['link'])) {
			?></a><?php
		} else {
			?></div><?php
	...
	?><div class="sc_price_info">
		...
		// Label
		if (!empty($args['label'])) {
			?><div class="sc_price_item_label popular"><?php echo esc_html($args['label']); ?></div><?php
		}
		...
		if (!empty($args['price'])) {
			...
			?><div class="sc_price_price"><?php
				...
				if (count($parts) > 1 && $parts[2]!='') {
					?><span class="sc_price_format"><?php echo wp_kses_post($parts[2]); ?></span><?php
				}
			?></div><?php
		}
		?>
