...
if (!function_exists('translang_customizer_get_css')) {

	function translang_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = translang_customizer_add_theme_fonts($fonts);
...
CSS;
			$rez = apply_filters('translang_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
...
CSS;
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(translang_get_list_schemes()) : array($only_scheme);

			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = translang_get_scheme_colors($scheme);

					// Make theme-specific colors and tints
					$colors = translang_customizer_add_theme_colors($colors);
...
input[type="button"],
.more-link,
.scheme_self.comments_wrap .form-submit input[type="submit"],
/* BB & Buddy Press */
...
.translang_tabs .translang_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
.translang_tabs .translang_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.post_layout_sticky .excerpt_content_wrapper_top .post_meta a,
.post_layout_excerpt .excerpt_content_wrapper_top .post_meta a{
	color: {$colors['bg_color']}; 	
}
...
.post_layout_excerpt .post_content .post_content_inner + .post_meta a,
.post_layout_excerpt .post_content .post_content_inner + .post_meta .post_meta_item:before{
	color: {$colors['text_hover']};
}
.post_layout_excerpt .post_content .post_content_inner + .post_meta a:hover,
.post_layout_excerpt .post_content .post_content_inner + .post_meta .post_meta_item:hover:before{
	color: {$colors['text_link']};
}
...
.scheme_self.sidebar[class*="scheme_"] .widget {
	background-color: {$colors['alter_bg_hover']};
}
...
.sidebar.scheme_self .widget .widget_title {
	background-color: {$colors['alter_text']};
}
...
.footer_wrap .scheme_self.vc_row .trx_addons_accent > *,
.footer_wrap .scheme_self.vc_row .trx_addons_accent > a {
...
big span.trx_addons_accent label { color: {$colors['text_hover']};}
.dark_link { color: {$colors['text_dark']};}
.dark_link a { color: {$colors['text_dark']};}
.dark_link a:hover { color: {$colors['text_link']};}

CSS;

					$rez = apply_filters('translang_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'translang_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>
